...
int alive = 1, x, y, gameSpeed = 200,Rokis_score = 0, dirX, dirY, oldBodyCor[myY*myX][2], body[myY*myX][2], numOfBody = 0;
...
int main() {
    ...
    do {
        ...
        while (1) {
            goToXY(0, 0);
            printf("ROKI YOU DIED");
            printf("\nROKIs FINAL SCORE : %d",Rokis_score);
            printf("\nRoki Do you want to try again?[y/n]");
            key = _getch();
            if (key == 'y') {
                alive = 1;
                reset();
...
void draw() {
    ...
    goToXY(0, 0);
    printf("Rokis_Score : %d",Rokis_score);
}
void move() {
    for (y = 0; y < myY; ++y) {
        for (x = 0; x < myX; ++x) {
